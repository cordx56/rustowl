...
COPY . .
RUN cargo xtask toolchain cargo install cargo-chef --locked

FROM chef AS planner
RUN cargo xtask toolchain cargo chef prepare --recipe-path recipe.json

FROM chef AS builder
COPY --from=planner /app/recipe.json recipe.json
RUN cargo xtask toolchain cargo chef cook --release --recipe-path recipe.json
RUN cargo xtask toolchain cargo build --release
