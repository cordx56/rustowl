...
mod miri_memory_safety_tests {
    ...
    fn test_workspace_operations() {
        ...
        crate1.0.insert("lib.rs".to_string(), File { items: Vec::new() });
        crate1.0.insert("main.rs".to_string(), File { items: Vec::new() });
        
        crate2.0.insert("helper.rs".to_string(), File { items: Vec::new() });
    ...
    fn test_mir_variables_operations() {
        ...
        mir_vars.push(user_var.clone());
        mir_vars.push(other_var.clone());
        ...
        let has_user_var = vars_vec.iter().any(|v| matches!(v, MirVariable::User { index: 1, .. }));
        let has_other_var = vars_vec.iter().any(|v| matches!(v, MirVariable::Other { index: 2, .. }));
    ...
    fn test_function_model_complex_operations() {
        ...
        assert_eq!(function.basic_blocks.len(), function_clone.basic_blocks.len());
    ...
    fn test_serialization_structures() {
        ...
        let fn_local_clone = fn_local.clone();
